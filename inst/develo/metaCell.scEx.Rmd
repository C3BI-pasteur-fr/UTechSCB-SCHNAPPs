...
```{r}
library("metacell")
if(!dir.exists("metaDataDbscEx")) dir.create("metaDataDbscEx/")
scdb_init("metaDataDbscEx", force_reinit=T)
# mcell_import_scmat_10x("test", base_dir="http://www.wisdom.weizmann.ac.il/~atanay/metac_data/pbmc_8k/")

# wget http://www.wisdom.weizmann.ac.il/~atanay/metac_data/pbmc_8k//matrix.mtx
# wget http://www.wisdom.weizmann.ac.il/~atanay/metac_data/pbmc_8k//features.tsv
# wget http://www.wisdom.weizmann.ac.il/~atanay/metac_data/pbmc_8k//barcodes.tsv

cp = load("~/Rstudio/scShinyHubData/GSE168704.schnapps.RData")
...
if(!dir.exists("metaDataScExFigs")) dir.create("metaDataScExFigs/")
scfigs_init("metaDataScExFigs/")
...
ig_genes = c(grep("^IGJ|^IGH", nms, value = T), 
                grep("^IGH",nms,value = T),
                grep("^IGK", nms, value =T), 
                grep("^IGL", nms, value =T))

bad_genes = unique(c(grep("^MT-", nms, value = T), grep("^MTMR", nms, value = T), grep("^MTND", nms, value = T),"NEAT1","TMSB4X", "TMSB10", ig_genes))
...
* not needed for schnapps as it will use the scEx object that is already filtered.
